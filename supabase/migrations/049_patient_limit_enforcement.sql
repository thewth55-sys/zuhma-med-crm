...
--   - trial and salud_pro are uncapped (NULL) — trial because
...
--     (PATIENT_LIMIT) so the app can detect this specific
...
BEGIN
  ...
  IF v_limit IS NOT NULL THEN
    ...
    IF v_count >= v_limit THEN
      RAISE EXCEPTION 'PATIENT_LIMIT: this account has reached its plan''s patient limit (%)', v_limit;
    END IF;
  END IF;
