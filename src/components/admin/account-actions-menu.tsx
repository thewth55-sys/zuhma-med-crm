...
export function AccountActionsMenu({
  ...
  async function handleDeleteDemo() {
    if (
      !confirm(
        `¿Eliminar la cuenta demo "${accountName}"? Se borran sus datos, usuarios y todo su contenido. Esta acción no se puede deshacer.`,
      )
    )
      return;
  ...
  async function handleImpersonateConfirm() {
    ...
    try {
      ...
      // El admin sigue siendo él mismo aquí: capturamos su id para
      // atribuirle la sesión de impersonación, y estampamos los
      // marcadores ANTES de verifyOtp para que el SIGNED_IN resultante
      // (que dispara log-session) ya los vea. Ver impersonation-flag.ts.
      const {
        data: { user: adminUser },
