...
import { IMPERSONATION_SESSION_FLAG, IMPERSONATOR_ID_KEY } from "@/lib/auth/impersonation-flag";
...
interface Member {
  ...
  /** Plataformas de push registradas por este usuario (android/ios/web). Vacío = sin app para push. */
  pushDevices: string[];
...
export function AccountDetailPanel({ accountId }: { accountId: string }) {
  ...
  // Impersonar un miembro (subcuenta) específico — reemplaza la sesión del
  // admin. Marca la sesión como impersonación y la atribuye al admin.
  async function handleImpersonateMember(member: Member) {
    if (
      !confirm(
        `¿Impersonar a ${member.fullName ?? member.email ?? "este usuario"}? Reemplaza tu sesión de admin actual; para volver, cierra sesión y entra con tu cuenta.`,
      )
    )
      return;
    ...
    try {
      const res = await fetch(`/api/platform-admin/accounts/${accountId}/impersonate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: member.userId }),
      });
      const body = await res.json().catch(() => null);
      ...
      if (adminUser?.id) sessionStorage.setItem(IMPERSONATOR_ID_KEY, adminUser.id);
      ...
      if (error) {
        sessionStorage.removeItem(IMPERSONATION_SESSION_FLAG);
        sessionStorage.removeItem(IMPERSONATOR_ID_KEY);
            ...
            <div key={member.userId} className="flex items-center justify-between text-sm">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <button
                  ...
                  className="text-xs text-primary underline decoration-dotted hover:text-primary/80 disabled:opacity-50"
                >
                  {impersonatingId === member.userId ? "Entrando…" : "Impersonar"}
                </button>
                <button
                  type="button"
