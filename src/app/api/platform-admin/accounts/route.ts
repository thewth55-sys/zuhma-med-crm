...
export async function GET() {
  try {
    ...
    const { data: accounts, error: accountsErr } = await admin
      .from("accounts")
      .select(
        "id, name, owner_user_id, plan, subscription_status, trial_ends_at, included_seats, portal_client_id, created_at, is_demo",
      )
    ...
    // El equipo interno (platform_admins) obtiene una cuenta propia vía
    // handle_new_user, pero NO es una cuenta cliente — solo debe vivir en
    // "Equipo interno", no en este listado. Los excluimos por dueño.
    const { data: platformAdmins } = await admin.from("platform_admins").select("user_id");
    const internalUserIds = new Set((platformAdmins ?? []).map((a) => a.user_id as string));
    const clientAccounts = (accounts ?? []).filter(
      (a) => !a.owner_user_id || !internalUserIds.has(a.owner_user_id),
    );
    ...
    const result = clientAccounts.map((account) => {
