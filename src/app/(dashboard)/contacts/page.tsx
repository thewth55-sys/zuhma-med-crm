...
export default function ContactsPage() {
  ...
  // (migración 038: "a contact becomes a patient when this row is
  // created"). ON por defecto: Pacientes muestra solo pacientes
  // ADQUIRIDOS; los prospectos/leads viven en Prospectos (el pipeline).
  // Se puede apagar para ver también los no convertidos.
    ...
    <div className="space-y-6">
      ...
      <ContactForm
        ...
        onSaved={async (contactId) => {
          // "Agregar paciente": un contacto NUEVO se crea como paciente
          // ADQUIRIDO (se le genera el perfil médico) para que aparezca en
          // Pacientes. En edición no se toca. Los leads sin perfil viven en
          // Prospectos (el pipeline).
          if (!editContact && accountId && contactId) {
            const { error } = await supabase
              .from('patient_profiles')
              .insert({ account_id: accountId, contact_id: contactId });
            if (error && error.code !== '23505') {
              console.error('[contacts] convert-to-patient failed:', error);
            }
          }
          fetchContacts();
          fetchTags();
