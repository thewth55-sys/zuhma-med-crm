...
import { clearNativeSession, isNativePlatform } from "@/lib/native-session";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (currentUser) {
        ...
        if (event === "SIGNED_IN") {
          // Si venimos de una impersonación (menú de admin estampó los
          // marcadores antes de verifyOtp), etiquetamos la sesión y la
          // atribuimos al admin en vez de al cliente. Ver impersonation-flag.ts.
          let impersonation = false;
          let impersonatorId: string | null = null;
          try {
            impersonation = sessionStorage.getItem(IMPERSONATION_SESSION_FLAG) === "1";
            impersonatorId = sessionStorage.getItem(IMPERSONATOR_ID_KEY);
            if (impersonation) {
              sessionStorage.removeItem(IMPERSONATION_SESSION_FLAG);
              sessionStorage.removeItem(IMPERSONATOR_ID_KEY);
            }
          } catch {
            // sessionStorage puede no existir (SSR / navegador restringido)
          }
          void fetch("/api/auth/log-session", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ impersonation, impersonatorId, native: isNativePlatform() }),
          }).catch(() => {});
        }
      } else {
        lastFetchedUserIdRef.current = null;
  ...
  const signOut = useCallback(async () => {
    const supabase = createClient();
    // Limpia la cookie de "dispositivo de confianza" (httpOnly) ANTES de
    // revocar la sesión, para que la próxima vez se pida el código 2FA.
    try {
